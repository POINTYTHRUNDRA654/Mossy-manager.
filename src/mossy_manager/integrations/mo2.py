    ...
    def _init_paths(self):
        ...
        if ini_file.exists():
                ...
                def detect_mo2_installation(cls) -> Optional[Path]:
                    ...
                    - Enumerates all subfolders in %LOCALAPPDATA%\ModOrganizer for valid ModOrganizer.ini (like LOOT)
                    - Accepts any folder containing ModOrganizer.exe as valid
                    Returns:
                    ...
                    # 2. Enumerate all subfolders in %LOCALAPPDATA%\ModOrganizer
                    ...
                    if modorganizer_root.exists():
                        for sub in modorganizer_root.iterdir():
                            if sub.is_dir():
                                ...
                                if ini_file.exists():
                                    # Try to extract the install path from the INI if possible
                                    try:
                                        config = configparser.ConfigParser()
                                        config.read(ini_file, encoding='utf-8')
                                        # If the INI has a [General] section with 'mo2_path', use it
                                    ...
                                    # Otherwise, treat the subfolder as a portable MO2 instance if it contains ModOrganizer.exe
                                    if (sub / 'ModOrganizer.exe').exists():
                                        logger.info(f"Found portable MO2 instance at: {sub}")
                                        return sub

                    logger.warning("Could not auto-detect MO2 installation (LOOT-style)")
                    return None
            config = configparser.ConfigParser()
            try:
